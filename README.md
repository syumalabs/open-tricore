...
A complete bare-metal C and C++ development flow for the TC4D7 on Linux, every layer validated on real silicon over the on-board DAP debugger. Current release is v2.1.
...
- Our own code runs on the PPU scalar core (Synopsys ARC EV71), a clean-room first, load and start ARC code, feed it input through LMU, and read results back, either straight out of the shared LMU at full bandwidth or over a run-state handshake, see [`ppu/`](ppu) and [`tools/tc-ppu`](tools/tc-ppu)
...
- An on-chip data flash (DFLASH) driver in the BSP, sector erase and 8-byte page programming on the AURIX's own data flash through the DMU command interface, giving persistent storage with no external memory, self-tested on chip by erasing a sector, programming a known pattern, and reading it back, see [`bsp/tc4d7`](bsp/tc4d7)
